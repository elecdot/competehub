...
const registrationAvailable = computed(() => auth.publicEmailRegistrationEnabled)
    ...
    <div class="auth-card">
      ...
      <div class="auth-form-panel">
        ...
        <div class="auth-tabs" aria-label="登录注册切换">
          ...
          <RouterLink v-if="registrationAvailable" class="active" to="/register">注册</RouterLink>
        </div>
        ...
        <AResult
          v-else-if="auth.capabilitiesLoaded"
          data-testid="register-unavailable"
          status="403"
          title="当前暂未开放自助注册"
          sub-title="请联系管理员，或使用学校预置的邮箱、学号、手机号登录。"
        >
          <template #extra>
            <RouterLink class="detail-link" to="/login">
              <LoginOutlined />
        ...
        <p v-if="registrationAvailable && currentStep !== 2" class="auth-switch-line">
          已有账号？
          <RouterLink to="/login">
            <LoginOutlined />
            去登录

...
| `message.ai-time-changed` | Read `competition_time_changed` message keyed by student, edition-scoped logical node key, and new node revision; immutable old/new time summary. | Historical read state, type filtering, and repeated-event idempotency. |

...
| D1-03 | Editor admin uses the governance workbench to create an unpublished赛事届次 and draft revision from a可信来源, then edits structured stages and paired nodes. | UI and API evidence shows `unpublished` lifecycle, `draft` revision status, source facts, completeness feedback, stage/node controls, and saved revision data. | `admin.day1@example.edu`; `competition.ai-challenge-published`; `revision.ai-challenge-v1`. | Competition Publication And Discovery |
...
| Logical node key | Revision / stage | Node type | Occurs at | Prominence | Purpose |
|---|---|---|---|---|---|
| `node.ai-registration-deadline` | `revision.ai-challenge-v1` / `stage.ai-registration` | `registration_deadline` | `2026-08-15T16:00:00Z` | `primary` | Public next node, calendar, and reminder creation. |
| `node.ai-submission-deadline` | `revision.ai-challenge-v1` / `stage.ai-submission` | `submission_deadline` | `2026-09-10T16:00:00Z` | `primary` | Subscription reminder generation. |
| `node.ai-competition-start` | `revision.ai-challenge-v1` / `stage.ai-competition` | `competition_start` | `2026-10-01T01:00:00Z` | `primary` | Calendar ordering. |
| `node.ai-2027-registration-deadline` | `revision.ai-challenge-2027-v1` / `stage.ai-2027-registration` | `registration_deadline` | `2027-08-15T16:00:00Z` | `primary` | Complete pending candidate. |
| `node.robotics-registration-deadline` | `revision.robotics-v1` / `stage.robotics-registration` | `registration_deadline` | `2026-08-20T16:00:00Z` | `primary` | Complete rejected candidate. |
| `node.math-registration-deadline` | `revision.math-v1` / `stage.math-registration` | `registration_deadline` | `2026-08-25T16:00:00Z` | `primary` | Retained offline snapshot. |
| `node.cancelled-registration-deadline` | `revision.cancelled-v1` / `stage.cancelled-registration` | `registration_deadline` | `2026-08-28T16:00:00Z` | `primary` | Retained cancelled snapshot. |
| `node.innovation-registration-deadline` | `revision.innovation-v1` / `stage.innovation-registration` | `registration_deadline` | `2026-08-30T16:00:00Z` | `primary` | General recommendation fallback. |
...
| `recommendation-daily-total.day1` | Item-level Shanghai-date aggregate by rule version/mode/position/actor kind/赛事; the request item contributes once despite multiple reasons. | D1-12 overall best-effort impression/click ratio without double counting. |
| `recommendation-reason-daily.day1` | Separate Shanghai-date attribution rows for each distinct displayed reason code on the request item. | D1-12 reason attribution evidence; rows are explicitly non-additive and non-causal. |

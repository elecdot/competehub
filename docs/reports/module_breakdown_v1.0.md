...
本项目采用模块 owner 负责制。模块 owner 的职责是保证对应模块的需求、接口、关键数据、测试方法和演示路径可以被清楚说明和持续协调；模块 owner 不等于独立完成该模块所有代码。具体开发时，以 GitHub Issue 作为迭代任务单位，在 Issue 或 PR 中明确 DRI、协作人、Reviewer、Validation 和 Docs impact。

这种分工用于保持每个核心模块有人解释和推进，同时避免把每个子模块都拆成过细的责任矩阵。跨模块工作按主流程汇合，优先打通学生和管理员核心闭环。
...
功能：
整理系统核心流程测试用例、接口测试记录、非功能验证记录、演示数据和答辩材料，保证系统在课程展示时能够完整演示主链路。

接口：
文档接口：`docs/testing.md`、测试用例文档、接口联调记录、非功能验证记录、演示流程文档。

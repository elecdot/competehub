...
| Favorite/Subscription | `POST/DELETE /competitions/{id}/favorite`、`POST/PATCH/DELETE /competitions/{id}/subscription` | 收藏和订阅。 |
...
系统测试与非功能验证模型以 `docs/testing.md` 为工程依据，课程报告中保留与
设计相关的测试分层和验收口径。
...
| 层级 | 测试对象 | 主要方法 |
|---|---|---|
| 后端单元测试 | services、repositories、规则推荐、状态流转、幂等规则。 | 使用 `pytest`，对业务规则和高风险状态变化优先采用 TDD。 |
| API 测试 | 认证、赛事搜索、订阅提醒、后台审核、消息、权限边界。 | 使用 Flask test client 覆盖请求校验、响应结构、状态码和权限。 |
| 数据库与迁移检查 | SQLAlchemy models、迁移脚本、seed 数据、状态枚举变更。 | 涉及 schema 变化时在临时或本地数据库执行迁移，seed 数据保持可复现。 |
| 集成测试 | 管理员发布赛事、学生搜索订阅、提醒生成消息和日历节点。 | P1 主闭环稳定后补自动化集成测试，自动化前使用手工验收脚本。 |
| 前端静态检查 | Vue route、TypeScript 类型、构建产物。 | 使用 `just web-lint` 和 `just web-build`。 |
| 前端组件测试 | 筛选栏、详情状态、订阅状态、消息状态。 | P1 UI 稳定后引入 Vitest 或等价组件测试框架。 |
| E2E 与手工验收 | 中期检查和答辩演示主流程。 | 先按脚本手工验收，路由和 API 稳定后再考虑 Playwright。 |

## 8.2 非功能验证口径

| 维度 | 验证重点 | 证据形式 |
|---|---|---|
| 权限安全 | 学生不能访问后台 API，用户不能读取或修改他人数据。 | API 权限用例，演示前角色切换检查。 |
| 数据一致性 | 状态变化同步影响列表、详情、推荐、订阅提醒和日历节点。 | 服务层或 API 状态流转测试。 |
| 幂等可靠性 | 提醒派发重试不生成重复站内消息，失败任务可重试。 | 服务层幂等测试或联调记录。 |
| 响应时间 smoke | 常规种子数据下列表、搜索和详情满足 3 秒目标。 | 记录种子数据规模、本地环境和访问结果。 |
| 易用性 | 老师或同学可按脚本完成学生和管理员主流程。 | 手工验收脚本执行记录。 |
| 可维护性 | 代码检查、文档同步和 PR 验证证据完整。 | `just check` 或相关分层命令、PR 检查清单。 |

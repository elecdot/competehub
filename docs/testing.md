...
| Database and migration checks | SQLAlchemy models, migration scripts, seed data, and state enum changes. | Run migrations against a temporary or local database when schema changes become concrete. Keep seed data reproducible. | Migration command output or a documented skipped reason when no schema changed. |

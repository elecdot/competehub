...
- Current static checks：`vue-tsc --noEmit`，通过 `just web-lint` 执行。
- Unit tests：工具函数、stores、关键组件。测试框架在前端测试任务具体落地时补充。
- Component tests：赛事筛选、详情状态、消息状态。
- E2E tests：核心查赛链路、订阅提醒链路、后台审核链路。
- Lint / Format：若后续引入 ESLint、Prettier 或组件测试框架，应同步更新 `apps/web/package.json`、`justfile` 和本文档。

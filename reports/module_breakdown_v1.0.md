...
| d  | M5 规则推荐与适配说明模块    | M1 用户画像、M3 赛事标签展示     | 推荐规则、推荐理由、适配标签、价值说明      |
...
功能：
基于学生画像、赛事标签、适合专业、适合年级、截止时间和系统配置权重生成个性化推荐结果，并展示可解释的推荐理由。推荐结果应保持可追溯，不使用不可解释的绝对价值评分作为赛事价值判断。
...
功能：
保证推荐页中的推荐理由与详情页中的适配标签、价值依据说明保持一致，避免出现推荐理由和赛事详情信息互相矛盾的情况。

接口：
页面接口：推荐理由标签、详情页适配标签、价值依据说明区。
内部服务接口：适配说明生成服务、标签展示服务。
数据接口：`competition_tags`、`competition_tag_links`、`competitions.value_notes`。
...
M5 规则推荐与适配说明模块基于 M1 用户画像、M2/M3 赛事标签和 M6 推荐规则配置生成推荐结果和推荐理由。
